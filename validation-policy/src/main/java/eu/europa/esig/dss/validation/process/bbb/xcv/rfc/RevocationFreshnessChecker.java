...
public class RevocationFreshnessChecker extends Chain<XmlRFC> {
	...
	protected void initChain() {
		 ...
		 * maximum accepted revocation freshness and the revocation information
		 * status is a CRL or an OCSP response IETF RFC 5280 [1], IETF RFC 6960
		 * [i.12] with a value in the nextUpdate field the time interval between
		 * the fields thisUpdate and nextUpdate shall be used as the value of
		 * maximum freshness. If nextUpdate is not set, the building block shall
		...
		/*
		 * 2) If the issuance time of the revocation information status is after
		 * the validation time minus the considered maximum freshness, the
		 * building block shall return the indication PASSED. Otherwise the
