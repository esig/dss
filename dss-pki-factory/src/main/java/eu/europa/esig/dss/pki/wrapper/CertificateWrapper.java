...
public class CertificateWrapper {
    ...
    private final CertificateType cert;
    private final EntityId key;

    public CertificateWrapper(CertificateType cert, String issuerName) {
        this.cert = cert;
    ...
    public EntityKey getAIA() {
        return cert.getAia();
    }

    public EntityKey getOCSP() {
        return cert.getOcsp();
    }
    ...
    private EntityId convert(EntityKey key) {
        if (key != null) {
    ...
    public KeyUsage getKeyUsage() {
        CertificateType.KeyUsages keyUsages = cert.getKeyUsages();
    ...
    private Date convert(DateDefinitionType ddt) {
        if (ddt != null) {
    ...
    public ASN1Sequence getQCStatementsIds() {
        if ((cert.getQcStatementIds() != null && !CollectionUtils.isEmpty(cert.getQcStatementIds().getQcStatement()))
                || (cert.getQcTypes() != null && !CollectionUtils.isEmpty(cert.getQcTypes().getQcType()))) {

            ASN1EncodableVector vector = new ASN1EncodableVector();

            // QC Statements IDs
            if (cert.getQcStatementIds() != null
                    && !CollectionUtils.isEmpty(cert.getQcStatementIds().getQcStatement())) {
            ...
            if (cert.getQcTypes() != null && !CollectionUtils.isEmpty(cert.getQcTypes().getQcType())) {
                ...
                QCStatement qcTypes = new QCStatement(ETSIQCObjectIdentifiers.id_etsi_qcs_QcType,
                        new DERSequence(typeVector));
            ...
            if (cert.getQcCClegislation() != null && !CollectionUtils.isEmpty(cert.getQcCClegislation().getCountryName())) {
                ...
                QCStatement qcCClegislation = new QCStatement(new ASN1ObjectIdentifier(QCStatementOids.QC_CCLEGISLATION.getOid()),
                        new DERSequence(cclegislationVector));
