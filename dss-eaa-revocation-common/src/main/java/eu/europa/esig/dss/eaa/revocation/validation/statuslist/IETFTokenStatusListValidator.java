...
import eu.europa.esig.dss.eaa.revocation.validation.AttestationRevocationValidator;
import eu.europa.esig.dss.eaa.revocation.x509.AttestationRevocationListCertificateSource;
...
public class IETFTokenStatusListValidator implements AttestationRevocationValidator {
    ...
    public List<String> getUris(Attestation attestation) {
        ...
        ClaimString uriClaim = attestation.getPayload().getStatus().getStatusList().getUri();
    ...
    public AttestationRevocationToken validate(Attestation attestation, byte[] statusListDocument) {
        ...
        ClaimNumber indexClaim = attestation.getPayload().getStatus().getStatusList().getIndex();
            ...
            if (validatorOptions.hasNext()) {
                for (StatusListValidatorFactory factory : loader) {
                    if (factory.isSupported(statusListDocument)) {
                        ...
                        AttestationRevocationToken statusToken = tokenStatusListValidator.getRevocationToken(eaaIndex);
