...
public class XAdESSignatureEn319132_Extends {
	...
	public void extendsBToT() throws Exception {
		...
		Assert.assertEquals(extendedReport.getSimpleReport().getIndication(extendedReport.getSimpleReport().getFirstSignatureId()), "INDETERMINATE");
	}
	...
	private DSSDocument createSignedDocument(SignatureLevel level) throws Exception {
		...
		Assert.assertEquals(report.getSimpleReport().getIndication(report.getSimpleReport().getFirstSignatureId()), "INDETERMINATE");
