...
public abstract class ExtensionBuilder extends XAdESBuilder {
	...
	protected void ensureUnsignedProperties() {
		...
		if (length == 1) {
			...
			unsignedPropertiesDom = DomUtils.addElement(documentDom, qualifyingPropertiesDom, DSSNamespaces.XADES_132.getUri(), XADES_UNSIGNED_PROPERTIES);
	...
	protected void ensureUnsignedSignatureProperties() {
		...
		if (length == 1) {
			...
			unsignedSignaturePropertiesDom = DomUtils.addElement(documentDom, unsignedPropertiesDom, DSSNamespaces.XADES_132.getUri(), XADES_UNSIGNED_SIGNATURE_PROPERTIES);
			if (params.isPrettyPrint()) {
