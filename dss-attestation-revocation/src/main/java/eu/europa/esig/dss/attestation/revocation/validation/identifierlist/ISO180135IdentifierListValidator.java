...
/**
 * Verifies attestation's revocation status using the Identifier List mechanism as defined
...
public class ISO180135IdentifierListValidator implements AttestationRevocationValidator {
    ...
    public List<String> getUris(Attestation attestation) {
        if (!isSupported(attestation)) {
            throw new UnsupportedOperationException("The provided attestation token does not contain 'identifier_list' or not supported!");
        }
        VerifiedClaimString uriClaim = attestation.getPayload().getStatus().getIdentifierList().getUri();
    ...
    public AttestationRevocationToken validate(Attestation attestation, byte[] identifierListDocument) {
        ...
        VerifiedClaimByteString identifier = attestation.getPayload().getStatus().getIdentifierList().getIdentifier();
