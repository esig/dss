...
/**
 * Requests attestation Revocation for an attestation revocation verification using the online data loader.
 * This class uses a ServiceLoader to load a relevant validator based on the mechanism specified in the attestation's body.
 * To specify a list of supported mechanisms, please provide a list of corresponding classes in 
 * /resources/META-IN/services/eu.europa.esig.dss.attestation.revocation.validator.AttestationRevocationValidator file.
 *
 */
public class OnlineAttestationRevocationSource implements AttestationRevocationSource {
    ...
    public OnlineAttestationRevocationSource() {
        ...
        LOG.trace("+OnlineAttestationRevocationSource with the default data loader.");
    }
    ...
    public OnlineAttestationRevocationSource(final DataLoader dataLoader) {
        ...
        LOG.trace("+OnlineAttestationRevocationSource with the specific data loader.");
    }
     ...
     *
     * @param dataLoader
     *            the component that allows to retrieve a attestation Token Status List (TSL) response using HTTP.
     */
    ...
    public AttestationRevocationToken getAttestationRevocation(Attestation attestation) {
        ...
        LOG.trace("--> OnlineAttestationRevocationSource queried for {}", attestation.getId());
        ...
        if (validator != null) {
            try {
                ...
                LOG.warn("An error occurred on attestation revocation validation using the {} : {}", validator.getClass().getSimpleName(), e.getMessage());
            }
        }
        return null;
    }
    ...
    /**
     * Performs validation of the {@code Attestation} using the {@code validator}
     *
    ...
    protected AttestationRevocationToken validate(Attestation attestation, AttestationRevocationValidator validator) {
        ...
        for (String uriLocation : uris) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Trying to retrieve an attestation revocation from URL '{}'...", uriLocation);
            }
            nbTries--;
            ...
            try {
                ...
                if (nbTries == 0) {
                    throw new DSSExternalResourceException(String.format(
                            "Unable to retrieve attestation revocation response for attestation with Id '%s' from URL '%s'. Reason : %s",
                            attestation.getId(), uriLocation, e.getMessage()), e);
                } else {
                    LOG.warn("Unable to retrieve attestation revocation response with URL '{}' : {}", uriLocation, e.getMessage());
                }
            }
        }

        throw new IllegalStateException(String.format("Invalid state within OnlineAttestationRevocationSource " +
                "for a attestation revocation call with id '%s'", attestation.getId()));
    }
