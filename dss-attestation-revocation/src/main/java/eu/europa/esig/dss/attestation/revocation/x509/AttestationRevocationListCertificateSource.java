...
import eu.europa.esig.dss.model.attestation.claim.VerifiedClaimRevocationList;
...
/**
 * Contains certificates extracted from a Token Status List claim present in the attestation payload
 *
 */
public class AttestationRevocationListCertificateSource extends TokenCertificateSource {
    ...
    private final VerifiedClaimRevocationList claimRevocationList;

    /**
     * Default constructor
     *
     * @param claimRevocationList {@link VerifiedClaimStatus}
     */
    public AttestationRevocationListCertificateSource(final VerifiedClaimRevocationList claimRevocationList) {
    ...
    private void extractCertificates() {
        VerifiedClaimByteString certificateByteString = claimRevocationList.getCertificate();
        if (certificateByteString != null) {
            try {
                CertificateToken certificate = DSSUtils.loadCertificate(certificateByteString.getBinaryValue());
                addCertificate(certificate, CertificateOrigin.ATTESTATION);
            } catch (Exception e) {
