...
/**
 * Validates attestation revocation, with a declared structure as defined in ETSI TS 119 472-1 v1.2.1 "5.2.10 attestation revocation service".
 *
 */
public class ETSI194721V121TokenStatusListValidator implements AttestationRevocationValidator {
    ...
    public List<String> getUris(Attestation attestation) {
        if (!isSupported(attestation)) {
            throw new UnsupportedOperationException("The provided attestation token does not contain 'status' or not supported!");
        }
        VerifiedClaimString uriClaim = attestation.getPayload().getStatus().getUri();
    ...
    public AttestationRevocationToken validate(Attestation attestation, byte[] statusListDocument) {
        ...
        VerifiedClaimNumber indexClaim = attestation.getPayload().getStatus().getIndex();
            ...
            if (validatorOptions.hasNext()) {
                for (TokenStatusListValidatorFactory factory : loader) {
                    if (factory.isSupported(statusListDocument)) {
                        ...
                        return tokenStatusListValidator.getRevocationToken(attestationIndex);
                    }
                }
            }
