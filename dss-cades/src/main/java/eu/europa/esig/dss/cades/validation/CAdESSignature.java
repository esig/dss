...
public class CAdESSignature extends DefaultAdvancedSignature {
	 ...
	 *            an expanded SignerInfo block from a CMS Signed
	 *            message
	 */
	...
	/**
	 * ETSI TS 101 733 V2.2.1 (2013-04) 5.6.3 Signature Verification Process
	...
	private void parseSignerAttribute() {
		...
		boolean signerAttrV2 = false;
		Attribute attribute = getSignedAttribute(PKCSObjectIdentifiers.id_aa_ets_signerAttr);
		if (attribute == null) {
			attribute = getSignedAttribute(OID.id_aa_ets_signerAttrV2);
			if (attribute == null) {
				return;
			}
			signerAttrV2 = true;
		...
		final ASN1Encodable attrValue = attrValues.getObjectAt(0);
		try {
			ASN1Sequence asn1Seq = ASN1Sequence.getInstance(attrValue);
			for (int i = 0; i != asn1Seq.size(); i++) {
				ASN1TaggedObject taggedObject = ASN1TaggedObject.getInstance(asn1Seq.getObjectAt(i));
				if (taggedObject.getTagNo() == 0) {
					parseClaimedAttributes(taggedObject);
				} else if (taggedObject.getTagNo() == 1) {
					if (signerAttrV2) {
						parseCertifiedAttributesV2(taggedObject);
					} else {
			...
			LOG.error("Error when parsing attribute : [" + attribute.getAttrType() + "]", e);
		}
	}

	private void parseCertifiedAttributesV2(ASN1TaggedObject taggedObject) {
		ASN1Sequence asn1Seq = ASN1Sequence.getInstance(taggedObject, true);
		for (int i = 0; i != asn1Seq.size(); i++) {
			ASN1TaggedObject taggedObj = ASN1TaggedObject.getInstance(asn1Seq.getObjectAt(i));
			if (taggedObject.getTagNo() == 0) {
	...
	private void parseCertifiedAttributes(ASN1TaggedObject taggedObject) {
		AttributeCertificate attributeCertificate = AttributeCertificate.getInstance(ASN1Sequence.getInstance(taggedObject, true));
	...
	/**
	 * This method handles the archive-timestamp-v2 The value of the
	 * messageImprint field within TimeStampToken shall be a hash of the
	 * concatenation of: • the encapContentInfo element of the SignedData
	 * sequence; • any external content being protected by the signature, if the
	 * eContent element of the encapContentInfo is omitted; • the Certificates
	 * and crls elements of the SignedData sequence, when present; and • all
	 * data elements in the SignerInfo sequence including all signed and
	 * unsigned attributes. NOTE 1: An alternative archiveTimestamp attribute,
	 * identified by an object identifier { iso(1) member-body(2) us(840)
	 * rsadsi(113549) pkcs(1) pkcs-9(9) smime(16) id-aa(2) 27, is defined in
	 * prior versions of TS 101 733. The archiveTimestamp attribute, defined in
	 * versions of TS 101 733 prior to 1.5.1, is not compatible with the
	 * attribute defined in the present document. The archiveTimestamp
	 * attribute, defined in versions 1.5.1 to 1.6.3 of TS 101 733, is
	 * compatible with the present document if the content is internal to
	 * encapContentInfo. Unless the version of TS 101 733 employed by the
	 * signing party is known by all recipients, use of the archiveTimestamp
	 * attribute defined in prior versions of TS 101 733 is deprecated. NOTE 2:
	 * Counter signatures held as countersignature attributes do not require
	 * independent archive time-stamps as they are protected by the archive
	 * time-stamp against the containing SignedData structure. NOTE 3: Unless
	 * DER is used throughout, it is recommended that the binary encoding of the
	 * ASN.1 structures being time-stamped be preserved when being archived to
	 * ensure that the recalculation of the data hash is consistent. NOTE 4: The
	 * hash is calculated over the concatenated data elements as received
	 * /stored including the Type and Length encoding. NOTE 5: Whilst it is
	 * recommended that unsigned attributes be DER encoded, it cannot generally
	 * be so guaranteed except by prior arrangement.
	 *
