...
After that specify a list of dependencies required for your project. See <<MavenModules>> to get familiar with the available modules in DSS.
...
WARNING: Some modules (e.g. `dss-utils`, `dss-crl-parser`, etc., see ch. <<SpecificModules>>) still have to be built completely when using the `quick` profile.
...
In order to generate HTML and PDF documentation for DSS project, the module `dss-cookbook` of DSS Core must be built with the following command (please, ensure that you are located in the `/dss-cookbook` directory):
...
* https://ec.europa.eu/cefdigital/DSS/webapp-demo/downloads/dss-app-minimal-windows-x64.zip[Minimal ZIP (application + bat file)];
* https://ec.europa.eu/cefdigital/DSS/webapp-demo/downloads/dss-app-complete-windows-x64.zip[Complete ZIP (application + bat file + OpenJDK + JavaFX SDK)].
...
Some modules of DSS framework have a specific behavior and has to be handled accordingly.
