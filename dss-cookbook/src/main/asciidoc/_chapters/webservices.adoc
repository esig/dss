...
* *EAA creation webservices* (`dss-attestation-creation-soap` / `dss-attestation-creation-rest`) and their clients: expose methods to allow remote EAA creation, by providing payload or payload configuration and signature parameters, disclosures extraction based on parameters, key binding signature creation and EAA Presentation issuance.
* *EAA validation webservices* (`dss-attestation-validation-soap` / `dss-attestation-validation-rest`) and their clients: expose methods to allow validation of EAA Presentations.
...
==== REST EAA creation service

This service implements a creation of EAAs, as well as issuance of EAA presentations.
...
Creation of an EAA may be done either based on a provided payload, or parameters used to define the payload to be generated.
The signature of an EAA is done in 3 stateless steps, similarly to a signature creation as explained in <<SignatureCreationInDSS>>.
...
The method allows generation of a signed EAA. The user sends the payload parameters (pre-computed payload or payload generation parameters), the signature parameters (signature level, signing-certificate, etc.) and the signature value.
...
The method allows retrieving of disclosures used to generate hashes for selectively disclosable claims within the signed EAA. The user provided payload parameters (same as the ones used in the steps above).
...
The method allows retrieving the data to be signed for a key binding signature generation. The user sends the signed EAA, (optionally) disclosures to be included in the EAA presentation, key binding signature payload parameters (e.g. nonce) and the signature parameters (signature level, signing-certificate, etc.).
...
The method allows generation of a key binding signature. The user sends the signed EAA, (optionally) disclosures to be included in the EAA presentation, key binding signature payload parameters (e.g. nonce), the signature parameters (signature level, signing-certificate, etc.) and a signature value.
...
The method allows generation of an EAA Presentation. The user sends the signed EAA, (optionally) disclosures to be included, (optionally) key binding signature and the parameters (format of EAA Presentation, etc.).
...
[[RestEAAValidationService]]
==== REST EAA validation service

This service performs a validation of EAA Presentations.

[source,java,indent=0]
.Rest EAA validation service
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/ws/rest/RestEAAValidationServiceSnippet.java[tags=demo]
----

===== EAA validation

This service allows a validation of an EAA Presentation, or EAAs of a certain format.
...
==== SOAP EAA creation service

SOAP EAA creation service's client is initialized using `SoapAttestationCreationService` class.
...
==== SOAP EAA validation service

SOAP EAA validation service's client is initialized using `SoapAttestationValidationService` class.

[source,java,indent=0]
.Soap EAA validation service
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/ws/soap/SoapEAAValidationServiceSnippet.java[tags=demo]
----
