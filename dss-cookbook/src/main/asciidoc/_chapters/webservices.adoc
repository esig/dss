...
* *EAA creation webservices* (`dss-attestation-creation-soap` / `dss-attestation-creation-rest`) and their clients: expose methods to allow remote attestation creation, by providing payload or payload configuration and signature parameters, disclosures extraction based on parameters, key binding signature creation and attestation Presentation issuance.
* *EAA validation webservices* (`dss-attestation-validation-soap` / `dss-attestation-validation-rest`) and their clients: expose methods to allow validation of attestation Presentations.
...
.Rest attestation creation service
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/ws/rest/RestEAACreationServiceSnippet.java[tags=demo]
----
...
The two methods, namely "get data to sign" and "sign EAA" are exposed to the API within the current module.
...
The method allows retrieving the data to be signed for a key binding signature generation. The user sends the signed EAA, (optionally) disclosures to be included in the attestation presentation, key binding signature payload parameters (e.g. nonce) and the signature parameters (signature level, signing-certificate, etc.).
...
The method allows generation of a key binding signature. The user sends the signed EAA, (optionally) disclosures to be included in the attestation presentation, key binding signature payload parameters (e.g. nonce), the signature parameters (signature level, signing-certificate, etc.) and a signature value.
...
The method allows generation of an attestation Presentation. The user sends the signed EAA, (optionally) disclosures to be included, (optionally) key binding signature and the parameters (format of attestation Presentation, etc.).
...
.Soap attestation creation service
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/ws/soap/SoapEAACreationServiceSnippet.java[tags=demo]
----
