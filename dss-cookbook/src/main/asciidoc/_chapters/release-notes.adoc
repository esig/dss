...
|v6.5              |August 2026             a|Main new features / improvements :

                                            * Attestation modules refactoring (`dss-sd-jwt`, `dss-mdoc`);
                                            * Cryptographic algorithms deprecation dates update per ETSI TS 119 312 v2.1.1 <<R20>>;
                                            * Support of `x5c` as unsigned header parameter (JAdES, CB-AdES);
                                            * RFC 9864 COSE algorithms validation support;
                                            * New `TLAlert` to detect and report invalid updates in `TSLSequenceNumber`;
                                            * Demos : added properties to enable certain alerts in web application;
                                            * Demos : added property to enforce RFC 8017 conformance on RSA signature validation (BC);
                                            * Demos : added property to configure maximum nesting level in ASN.1 structures (BC);
                                            * Demos : added LoTE validation job summary webpage;
                                            * Dependencies update (BouncyCastle, HttpClient5, PdfBox, etc.).

Bug fixes :

                                            * Fixes related to attestations handling (creation and validation);
                                            * Fixes related to LoTE processing;
                                            * `ExpirationAndSignatureCheckStrategy` mistakenly synchronizes Trusted Lists with zero or more signatures;
                                            * PAdES : `/AP` of existing signature field can be created without invalidating the signature;
                                            * JAdES : Performance issue in `JWSCompactSerializationParser#isSupported` method;
                                            * Missing commits for QWAC and crypto suites processing;
                                            * Outdated links to the eIDAS Dashboard in Simple Reports.

|v6.5.RC1          |July 2026              a|Main new features / improvements :

                                            * Creation and validation of CB-AdES signatures (ETSI TS 119 152-1 <<R41>>);
                                            * Creation of attestations and presentations in SD-JWT <<R35>> & ISO/IEC-mdoc <<R38>>;
                                            * Validation of attestations based on SD-JWT <<R35>> and ISO/IEC-mdoc <<R38>>, with support of ETSI TS 119-472-1 <<R34>>;
                                            * Attestation qualification validation (QEAA, PuB-EAA, PID, etc.);
                                            * Attestation revocation verification (Token Status List, Identifiers List);
                                            * Introduce generic validation job module for a document validation;
                                            * Validation job for a List of Trusted Entities (ETSI TS 119 602 <<R33>>), in XML and JSON;
                                            * Validation of certificate approval status using a LoTE (ETSI TS 119 605 <<R39>>);
                                            * REST and SOAP webservices for attestation creation and validation;
                                            * XAdES : Add alternative XPath query executor for XML processing;
                                            * PAdES : Review DPI behavior for visual PDF signatures;
                                            * PAdES : Resolve limitations in DSSSecureRansomProvider;
                                            * JAdES : Accept arbitrary `kid` protected header values;
                                            * JAdES : Add support of `x5c` as unsigned header parameter;
                                            * Support of ETSI TS 119 412-6 QcStatements;
                                            * Added a predicate to `FileCacheDataLoader` to choose whether to cache the downloaded data;
                                            * Dependencies update (BouncyCastle, HttpClient5, PdfBox, VeraPDF, etc.);
                                            * Demos : New page for CB-AdES signature creation;
                                            * Demos : New page in the webapp for validation of attestations;
                                            * Demos : OJEU keystore update;
                                            * Demos : Webapp upgraded to Spring-Boot 4;
                                            * Java 26 support.

|v6.5.RC1           |July 2026             a|Bug fixes :

                                            * Add missing i18n messages for validation indication semantics;
                                            * CAdES validation : fix extraction of timestamped references from `ats-hash-index` tables which are not `-v3`;
                                            * PAdES : Fix final image position computation when using the `ImageScaling.CENTER` with an image+text combination;
                                            * TL signature may be validated successfully against CA certificate;
                                            * TrustAnchorBPPolicy does not include signing-certificate when trusted;
                                            * Make `CertificateToken.checkIsSignedBy` thread safe;
                                            * Fixed eSignature validation tests for empty or corrupted documents.

|v6.4              |March 2026             a|Main new features / improvements :

                                            * LoTE schemas aligned with TS 119 602 v1.1.1 <<R33>>;
                                            * Updated default XML certificate validation policy;
                                            * Dependencies update (HttpClient5, etc.);
                                            * Documentation improvements;
                                            * DSS WebApplication : signature levels reviewed for signing operations.

Bug fixes :

                                            * Fixes related to 2-QWAC validation;
                                            * Fixed processing of `qc-identMethod` and `qc-qscdLegislation` QcStatements;
                                            * Incorrect behavior for some algorithm usages defined within a cryptographic suite;
                                            * Fixed printing of Algorithm Obsolescence Validation block within PDF Detailed Report;
                                            * DSS Standalone : changed the digest algorithm selector type to fix a button overflow issue.

|v6.4.RC1          |December 2025           a|Main new features / improvements :

                                            * QWAC Validator as per ETSI TS 119 411-5 <<R32>>;
                                            * Generic Signature Augmentation mechanism based on Service Loader;
                                            * Support of TS 119 602 <<R33>> Lists of trusted entities XML and JSON schemas;
                                            * Support of new QcStatements introduced in ETSI EN 319 412 v2.5.1 <<R28>>;
                                            * Extended support of ETSI TS 119 322 <<R21>> cryptographic suite properties;
                                            * Algorithm Obsolescence Check validation block;
                                            * Support of alternative security providers;
                                            * DSS Standalone bundle for Linux OS;
                                            * DSS Demonstrations updates (file cache revocation sources, sunset date, nonce, etc.);
                                            * Dependencies update (BouncyCastle, PdfBox, sKema, HttpClient5, etc.);
                                            * Java 25 support.

|v6.4.RC1          |December 2025           a|Bug fixes :

                                            * Incorrect URI Encoding in XAdES Detached Signature;
                                            * Enveloped countersignature ignored when Reference Type is missing;
                                            * Fix XAdES Archive Timestamp v1.3.2 message-imprint computation for enveloped signatures;
                                            * Inconsistent behavior when validating XML detached and manifest signatures with `DSSDocument` name attribute;
                                            * KeyUsage leads to `ArrayIndexOutOfBoundsException` when used with an alternative security provider;
                                            * `FileCacheOCSPSource` shall create OCSP key based on URL+Certificate combination;
                                            * `AIACertificateSource` ignores candidates when full certification path is returned;
                                            * XAdES and JAdES counter signatures are validated multiple times;
                                            * IssuerSerial not matching when DN has line breaks after non-space text ending;
                                            * Fixed wrong behavior on `PAdESService#addNewSignatureField` method usage;
                                            * Performance of timestamp's `ContainerSignedAndTimestampedFilesCovered` constraint;
                                            * ASiC augmentation issues (using detached content or embedded archive-time-stamp(-v3));
                                            * Validation fails when a third country Trust Service defines qualifiers, but no MRA mapping is provided;
                                            * PKI Factory : missing Authority Key Identifier certificate extension.
...
|v6.1.1          |March 2025               a|Main new features / improvements :
...
|v6.0.1          |March 2025               a|Main new features / improvements :
...
|v5.13.1          |March 2025               a|Main new features / improvements :
...
The example below shows how to switch to DSS version `6.5` using <<BomModule>>:
