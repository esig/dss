...
This method will search for an available `ValidationPolicyFactory` implementation at runtime by means of the <<ServiceLoaderAdvanced>>.
...
DSS provides, within its core code, a validation policy suitable for validating qualified electronic signatures (QES) and advanced electronic signatures (AES). This validation policy is loaded by default when a signature or certificate validation is initiated.

The default validation policy aims to enforce best practices for electronic signature creation and validation; however, its scope may not be exhaustive. It supports the validation of ETSI advanced electronic signature and container formats (i.e. *AdES formats) as well as other popular signature profiles, such as PKCS#7.

NOTE: DSS accepts no responsibility or liability whatsoever regarding the use of the provided validation policy. It is the responsibility of implementers to define a validation policy appropriate for their specific applicability framework.

The default XML validation policy is shown below.
...
The `DocumentValidator` will search for an available implementation of a `CryptographicSuiteFactory`, supporting the given cryptographic suite file, at runtime by means of the <<ServiceLoaderAdvanced>>.
...
The validation of cryptographic algorithms is performed according to the section "6. Processing" of the RFC 5698 "Data Structure for the Security Suitability of Cryptographic Algorithms (DSSC)". The full validation algorithm is described below.

Starting from DSS version 6.4, the following processing algorithm is applied when validating cryptographic algorithms against a defined cryptographic suite:

. All constraint entries in the cryptographic suite that match the cryptographic algorithm under validation (either by OID or URI) are extracted for further processing. If the obtained list is not empty, continue to the next step.
If no matching entries are found and the algorithm under validation is a signature algorithm, continue to step 5.
If none of the following applies, validation terminates with a failed status.

. All `Evaluation` entries from the extracted constraints are collected.
If the algorithm under validation is a digest algorithm, proceed to step 4.
Otherwise, proceed to the next step.

. If the cryptographic algorithm does not impose any parameters (e.g. SHA-1, SHA-256, etc. digest algorithms), continue to the next step.
If the cryptographic algorithm imposes verification of any parameters (such as key length, etc.), then from the collected `Evaluation` entries, retain only those containing a `Parameter` element with values matching the corresponding values of the cryptographic algorithm under validation. If no such entries remain, validation returns a failed status.
Otherwise, continue to the next step.

. The process verifies whether at least one of the filtered `Evaluation` entries either lacks an `End` element or has an `End` element (from its internal `Validity` element) with its datetime value after than the control time.
If such an entry exists, validation returns a successful status.
Otherwise, validation returns a failed status.

. For the digest algorithm used within the signature algorithm under validation, the processing algorithm is run from step 1.
If that validation returns a successful status, proceed to the next step.
Otherwise, validation returns a failed status.

. For the encryption algorithm used within the signature algorithm under validation, the process algorithm is run from step 1.
If that validation returns a successful status, the overall validation concludes with a successful status.
Otherwise, validation returns a failed status.

===== Algorithm Usage

Starting from version `6.4` DSS provides a support of an optional `AlgorithmUsage` element as defined within the ETSI TS 119 322 (cf. <<R21>>). Similarly to <<defaultXmlPolicy>>, the `AlgorithmUsage` element provides a possibility to define a scope for the applicability of the cryptographic rules. Below you may find a table of correspondence of different algorithm usage types to the supported contexts in the <<defaultXmlPolicy>>:

.Algorithm usage applicability table
[%header,cols="14*^.^", orientation=landscape]
|=====
|Algorithm usage              3+|Signature                                3+|Counter-signature                         3+|Revocation                               3+|Timestamp                                 |Evidence record
|                               |Signature  |Signing certificate  |CA       |Signature  |Signing certificate  |CA        |Signature  |Signing certificate  |CA       |Signature  |Signing certificate  |CA      |Data Objects
| sign data | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"]
| sign certificates | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] |
| sign OCSP | | | | | | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | | |
| sign timestamps | | | | | | | | | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] |
| validate data | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"]
| validate certificates | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] |
| validate OCSP | | | | | | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | | | |
| validate timestamps | | | | | | | | | | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] | icon:check-circle[role="lime"] |
|=====

NOTE: `sign OCSP` and `validate OCSP` algorithm usage types apply to both OCSP Responses and CRL revocation data types on validation with DSS.

As the use of `algorithmUsage` element is optional, it also stays possible to define a particular context and sub context of a cryptographic suite programmatically. For more details about the implementation, please see the <<ValidationPolicyLoader>> chapter.

===== Policy issue date

Policy issue date has an extended functionality in the DSS validation process than providing an information only. As such, similarly to the `UpdateDate` in the <<CryptographicConstraints>> of the DSS validation policy, the `PolicyIssueDate` element present within an XML or JSON Cryptographic Suite allows configuring a different validation behavior for algorithms expiring before and/or after the defined datetime.

As such, the following default rules apply:

* For cryptographic algorithms with the end of validity date before or at the datetime present within the `PolicyIssueDate` element, the validation would result to a `FAIL` level; and
* For cryptographic algorithms with the end of validity date after datetime present within the `PolicyIssueDate` element, the validation would result to a `WARN` level.

NOTE: This behavior can be useful to provide a "lax" deprecation mechanism for some cryptographic algorithms.

The default behavior can be configured with a custom validation level. Please see `ValidationPolicyLoader` with a cryptographic suite configuration provided in the <<ValidationPolicyLoader>> chapter.
...
In a simple use-case scenario, a default validation policy can be loaded, provided the implementation is discoverable and available to the application at runtime:
...
With the `ValidationPolicyLoader` verifying whether a module supporting the provided validation policy is available at runtime, and loading the policy using the first found implementation, supporting the given validation policy file.
...
* `COSEDocumentValidator` - validates documents in COSE (CBOR) format (CB-AdES format);
...
While any implementation of a `CertificateSource` can be used to provide a source of signing certificates, DSS provides a few format specific implementations, such as:

* `KidCertificateSource` - allow providing a collection of certificates by a unique `kid` (key identifier) value. The certificate source can be used for referencing certificates in JAdES and CB-AdES signature formats by the corresponding signed header parameter.
* `X509URLCertificateSource` - allow providing a collection of certificates by a unique `x5u` (X509 URL) value. The certificate source can be used for referencing certificates in JAdES and CB-AdES signature formats by the corresponding signed header parameter.
* `CommonX509URLCertificateSource` - Default implementation of the `X509URLCertificateSource` allowing manual configuration of the URL to certificate(s) mapping or dynamically by using a `DataLoader` (see an example below).
* `COSEX509URLCertificateSource` - Implementation of the `X509URLCertificateSource` allowing extraction of CBOR encoded certificates, as defined in RFC 9360 (used in CB-AdES).

[source,java,indent=0]
.Provide X509URLCertificateSource to the validation
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/SignedDocumentValidatorTest.java[tags=x509CertSource]
----
...
* `COSEDocumentValidatorFactory`: loads `COSEDocumentValidator`, used for a COSE (CBOR) validation (delivered in `dss-cb-ades` module);
...
==== Signature Policy Validator Loader

Signature Policy Validator Loader is used to define a behavior of loading a relevant <<SignaturePolicyValidator>>, e.g. based on the signature and/or signature policy technical specifications.

NOTE: This chapter represents advanced configurations of the validation process. Modifications within the parameter may result to a different validation result.

The chosen `SignaturePolicyValidatorLoader` can be provided directly within a `DocumentValidator` used for the signature validation, as shown below:

[source,java,indent=0]
.Signature Policy Validator Loader definition
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/SignedDocumentValidatorTest.java[tags=demo-sig-policy-loader]
----

Within the framework code, DSS delivers a `DefaultSignaturePolicyValidatorLoader` implementation, providing following options:

* *Signature based* signature policy validator loader : the default configuration, that will process the signature policy according to the rules defined in the corresponded AdES signature specification. E.g. when used in XAdES signatures, hash of the signature policy will be computed based on the de-referenced content of the signature policy obtained after processing `SigPolicyId` and `ds:Transforms`, with the exception to the signature policy identifiers definitions explicitly stating the "SPDocDigestAsInSpecification" transform (then the hash computation will be signature policy specific). No configuration should be changed to achieve this behavior.
* *Chosen validator first* signature policy validator loader : similar to the default configuration from above, but instead of applying the rules based on the signature format specification, it will use the provided "default" `SignaturePolicyValidator` to be loaded for a signature policy hash computation, unless a signature policy technical specification is explicitly specified by a "SPDocDigestAsInSpecification" transform or alternative parameter. The loader can be created by using the `DefaultSignaturePolicyValidatorLoader#defaultUnlessSpecifiedSignaturePolicyValidatorLoader` method.
* *Chosen validator only* signature policy validator loader : this loader will always use the provided "default" `SignaturePolicyValidator`, in all signature configurations, if set. The loader can be created by using the `DefaultSignaturePolicyValidatorLoader#defaultOnlySignaturePolicyValidatorLoader` method.
* *Signature Policy based* signature policy validator loader : will load the `SignaturePolicyValidator` implementation solely based on the technical specification of the signature policy, ignoring the signature specific requirements. The loader can be created by using the `DefaultSignaturePolicyValidatorLoader#policyBasedSignaturePolicyValidatorLoader` method.

If a more advanced configuration of the `SignaturePolicyValidator` loading process is required, a custom implementation of the `SignaturePolicyValidatorLoader` can be created.

=== Format specificities

==== XAdES

[[XPathQueryExpressions]]
===== XPath query expressions

Since version `6.5` DSS provides a customization of XPath queries execution, used on a XAdES processing for elements of interest lookup and extraction (mainly in augmentation and validation processes).

The configuration of the default behavior can be useful on processing of large XML documents with many XML nodes inside in order to improve performance of a XAdES augmentation and/or validation.

For customization of the behavior, DSS provides the following interfaces:

* `eu.europa.esig.dss.xml.utils.xpath.XPathQueryExecutor` - performs node extraction based on XPath expressions execution on XAdES processing, including augmentation and validation processes (less on signing, as it is required only occasionally).
* `eu.europa.esig.dss.xml.utils.xpath.XPathStringExecutor` - performs string XPath queries execution. Currently only used in pair with the `XAdESSignatureParameters#setXPathLocationString` method usage.

NOTE: The scope of the XPath executors is limited to the aforementioned components. As such, the XPath executors do not currently apply on XPath transform processing within a `ds:Reference` element.

Alternative implementations shall implement the target interface. Within DSS package the following implementations are provided:

* `JavaXmlXPathQueryExecutor` _(Default)_ - The XPath expressions processing implementation based on the `javax.xml.xpath` package classes. This implementation replicates the behavior on XPath processing in previous versions of DSS, before `6.4` (inclusive).
* `NativeDOMXPathQueryExecutor` _(Experimental)_ - Alternative implementation of XPath expressions handling, aiming to improve the performance of XPath queries execution. This implementation works on pure `org.w3c.dom` package elements, such as `Node` lookup and processing.

NOTE: DSS `6.5` uses the _legacy_ implementation of XPath expressions execution. We aim to switch the default implementation to the `NativeDOMXPathQueryExecutor` in the future version of DSS, thus improving XML processing performance out of the box.

DSS provides two ways of the implementation configuration:

1) Using a `ServiceLoader` _(recommended)_. A definition of the class implementation shall be provided in the folder `META-INF/services` within the root directory of the project, in a file with a name corresponding to the full path of the target interface. For instance, for a `XPathQueryExecutor` implementation configuration, the filename `eu.europa.esig.dss.xml.utils.xpath.XPathQueryExecutor` shall be used. The file itself shall contain a selected implementation, as shown below for an example:
[source,indent=0]
./META-INF/services/eu.europa.esig.dss.xml.utils.xpath.XPathQueryExecutor
----
eu.europa.esig.dss.xml.utils.xpath.NativeDOMXPathQueryExecutor
----

2) Using `XPathUtils` (for `XPathQueryExecutor` only). The chosen implementation can be provided directly using a corresponding setter within the `XPathUtils` class, as shown below:

[source,java,indent=0]
.XPathQueryExecutor configuration using XPathUtils
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/XPathSnippet.java[tags=demo]
----

NOTE: It is also possible to integrate a third-party dependency to be used within a custom alternative XPath executor implementation. As such, successful tests have been performed on using a `Saxon-HE` library for XPath execution.
