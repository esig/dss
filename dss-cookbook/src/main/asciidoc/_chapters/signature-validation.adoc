...
In case when intermediate certificates are not present within a signature document, nor in trusted/adjunct sources, a certificate chain can still be built using AIA URL obtained from a certificate. For more information on the Authority Information Access (AIA), refer to section <<AIA>>.
...
For information on how revocation of data is handled, see section <<RevocationDataMangementConcept, revocation data management concepts>> and chapter <<RevocationDataManagement>>.
...
The `CertificateVerifier` and its implementation `CommonCertificateVerifier` determines how DSS accesses the external resources and how it should react in some occasions. This object is used to provide four different sources of information:
...
Generally, a signature validation process outputs an indication status and a validation report as described in section <<SignatureValidationInputsOutputs>>.
...
A sample of a DetailedReport is provided https://github.com/esig/dss/blob/master/dss-cookbook/src/main/asciidoc/_samples/detailed-report-example.xml[here].
...
When reports are generated they are in the XML format, which is not the most straightforward way of reading a report. Therefore, stylesheets are used. A stylesheet is a set of rules that transforms XML content into an HTML or PDF representation to have a human-readable text.
...
==== Validation level
There exist four signature levels: `BASELINE-B`, `BASELINE-T`, `BASELINE-LT` and `BASELINE-LTA` (cf. <<SignatureClasses>>). For signature validation, DSS allows indicating which is the level of the signature that needs to be validated. The validation process can be for basic signatures, signatures with long-term validation data or signatures with archival data.
