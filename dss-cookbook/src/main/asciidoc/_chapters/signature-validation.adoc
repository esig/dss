...
==== Validation level
There exist four signature levels: `BASELINE-B`, `BASELINE-T`, `BASELINE-LT` and `BASELINE-LTA` (cf. <<SignatureClasses>>). For signature validation, DSS allows indicating which level of the signature needs to be validated. The validation process can be for basic signatures, signatures with long-term validation data or signatures with archival data.
...
The verification of points introduced above is managed by an instance of `PdfDifferencesFinder` provided within `PdfModificationDetectionUtils`. By default, an instance of `DefaultPdfDifferencesFinder` class is used. For an example of `PdfDifferencesFinder` configuration, please see below:

[source,java,indent=0]
.PdfDifferencesFinder customization
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/PAdESDisableVisualComparison.java[tags=visual-change-finder]
----
...
The search of the object differences is performed by an implementation of `PdfObjectModificationsFinder` provided within `PdfModificationDetectionUtils`. The categorization is done by an instance of `PdfObjectModificationsFilter` interface.
An example of a `PdfObjectModificationsFinder` customization is provided below:

[source,java,indent=0]
.PdfObjectModificationsFinder customization
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/snippets/PAdESDisableVisualComparison.java[tags=object-modifications]
----
