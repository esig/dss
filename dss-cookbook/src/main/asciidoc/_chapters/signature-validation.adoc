...
Since version `5.13` DSS provides a possibility of evidence records validation in relation to digital signatures or alone.

Currently, the only supported version of evidence records is RFC 6283 XML Evidence Records (cf. <<R22>>).
...
In order to add support of XML evidence records on validation, the following dependency has to be added to a `pom.xml` file within your project:
...
* `XMLEvidenceRecordValidatorFactory`: loads `XMLEvidenceRecordValidator`, used for an RFC 6283 XML Evidence Record (cf. <<R22>>) validation (delivered in `dss-evidence-record-xml` module).
