...
DSS allows generation, extension and validation of PAdES signatures with two different frameworks: PDFBox and OpenPDF (fork of iText). The `dss-pades` module only contains the common code and requires an underlying implementation :
...
By default, DSS uses the "Native Drawer" as the PDFBox implementation. In order to switch the implementation, that allowed in runtime, you have to set a new instance for PdfObjFactory as following:
...
It is possible to programatically create or/and edit an XML Trusted List using the JAXB module.
