...
To define the resources handler, the corresponding `DSSResourcesHandlerBuilder` has to be provided to the underlying object, e.g. to a `PAdESService` for a signature creation using the in temporary file processing:
