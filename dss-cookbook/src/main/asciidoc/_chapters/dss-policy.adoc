...
The `<Attestation>` block defines rules for validating an attestation (e.g. EAA, PID, etc.). The `<Attestation>` element shall be a child of `ConstraintsParameters`:

[source,xml,indent=0]
.Attestation element definition
----
<ConstraintsParameters>
    ...
    <Attestation>
...
* `AttestationSignatureUnicity` - this constraint checks whether the attestation contains a single signature, excluding the key binding signature.

Default: `FAIL`

[source,xml,indent=0]
.Example of AttestationSignatureUnicity usage (with FAIL validation level)
----
<Attestation>
    ...
    <AttestationSignatureUnicity Level="FAIL" />
    ...
</Attestation>
----

* `AttestationSignatureValid` - this constraint checks whether the signature protecting the attestation payload is valid and results in a TOTAL-PASSED validation status.

Default: `FAIL`

[source,xml,indent=0]
.Example of AttestationSignatureValid usage (with FAIL validation level)
----
<Attestation>
    ...
    <AttestationSignatureValid Level="FAIL" />
    ...
</Attestation>
----

* `DisclosurePresent` - this constraint checks whether the attestation presentation contains attached disclosures.
...
* `DisclosureFound` - this constraint checks whether a selectively disclosable claim hash matching the provided disclosure has been found within the attestation token.
...
* `DisclosureIntact` - this constraint checks whether the hash value computed from the provided disclosure matches the selectively disclosable claim hash contained within the attestation token.
...
* `DisclosureListExhaustive` - this constraint checks whether all selectively disclosable claim hashes defined within the attestation token have a corresponding disclosure attached to the attestation presentation.
...
* `KeyBindingSignaturePresent` - this constraint checks whether a key binding signature is present within the attestation token.
...
* `ETSI194721Conformance` - this constraint checks whether the attestation token is conformant to ETSI TS 119 472-1 (cf. <<R34>>).
...
* `AttestationType` - this constraint checks whether the attestation type is part of the list of accepted values. The wildcard value `*` can be used to accept any attestation type.

Default: `WARN`

[source,xml,indent=0]
.Example of AttestationType usage (with WARN validation level)
----
<Attestation>
    ...
    <AttestationType Level="WARN">
         <Id>*</Id>
    </AttestationType>
    ...
</Attestation>
...
* `AttestationTypeIntegrityPresent` - this constraint checks whether the vct#integrity claim is present within an SD-JWT VC attestation token.

Default: `WARN`

[source,xml,indent=0]
.Example of AttestationTypeIntegrityPresent usage (with WARN validation level)
----
<Attestation>
    ...
    <AttestationTypeIntegrityPresent Level="WARN" />
    ...
</Attestation>
----

* `IdentifierPresent` - this constraint checks whether an attestation identifier is present within the attestation token.

Default: not executed

[source,xml,indent=0]
.Example of IdentifierPresent usage (with WARN validation level)
----
<Attestation>
    ...
    <IdentifierPresent Level="WARN" />
    ...
</Attestation>
----

* `IssuanceDatePresent` - this constraint checks whether the issuance date is present within an ISO/IEC-mdoc attestation token.

Default: `FAIL`

[source,xml,indent=0]
.Example of IssuanceDatePresent usage (with FAIL validation level)
----
<Attestation>
    ...
    <IssuanceDatePresent Level="FAIL" />
    ...
</Attestation>
----

* `NotBeforePresent` - this constraint checks whether the `nbf` (not before) claim is present within the attestation token.

Default: `FAIL`

[source,xml,indent=0]
.Example of NotBeforePresent usage (with FAIL validation level)
----
<Attestation>
    ...
    <NotBeforePresent Level="FAIL" />
    ...
</Attestation>
----

* `ExpirationPresent` - this constraint checks whether the `exp` (expiration) claim is present within the attestation token.

Default: `FAIL`

[source,xml,indent=0]
.Example of ExpirationPresent usage (with FAIL validation level)
----
<Attestation>
    ...
    <ExpirationPresent Level="FAIL" />
    ...
</Attestation>
----

* `NotExpired` - this constraint checks whether the validation time falls within the attestation technical validity period defined by the `nbf` and `exp` claims.
...
* `AdministrativeIssuanceDatePresent` - this constraint checks whether the administrative issuance date is present within the attestation token.

Default: not execute

[source,xml,indent=0]
.Example of AdministrativeIssuanceDatePresent usage (with WARN validation level)
----
<Attestation>
    ...
    <AdministrativeIssuanceDatePresent Level="WARN" />
    ...
</Attestation>
----

* `AdministrativeExpirationDatePresent` - this constraint checks whether the administrative expiration date is present within the attestation token.

Default: not executed

[source,xml,indent=0]
.Example of AdministrativeExpirationDatePresent usage (with WARN validation level)
----
<Attestation>
    ...
    <AdministrativeExpirationDatePresent Level="WARN" />
    ...
</Attestation>
----

* `AdministrativePeriodNotExpired` - this constraint checks whether the validation time falls within the attestation administrative validity period defined by the `adm_nbf` and `adm_exp` claims.

Default: `WARN`

[source,xml,indent=0]
.Example of AdministrativePeriodNotExpired usage (with WARN validation level)
----
<Attestation>
    ...
    <AdministrativePeriodNotExpired Level="WARN" />
    ...
</Attestation>
----

* `Category` - this constraint checks whether the attestation category claim corresponds to one of the accepted values. The wildcard value `*` can be used to accept any category.

Default: not executed

[source,xml,indent=0]
.Example of Category usage (with FAIL validation level)
----
<Attestation>
    ...
    <Category Level="FAIL">
...
* `Subject` - this constraint checks whether the attestation subject claim corresponds to one of the accepted values. The wildcard value `*` can be used to accept any subject.

Default: not executed

[source,xml,indent=0]
.Example of Subject usage (with WARN validation level)
----
<Attestation>
    ...
    <Subject Level="WARN">
...
* `Pseudonym` - this constraint checks whether the attestation subject pseudonym claim corresponds to one of the accepted values. The wildcard value `*` can be used to accept any pseudonym.
...
* `IssuingCountry` - this constraint checks whether the attestation issuing country claim corresponds to one of the accepted values. The wildcard value `*` can be used to accept any issuing country.

Default: not executed

[source,xml,indent=0]
.Example of IssuingCountry usage (with WARN validation level)
----
<Attestation>
    ...
    <IssuingCountry Level="WARN">
        <Id>*</Id>
    </IssuingCountry>
    ...
</Attestation>
...
* `IssuingAuthority` - this constraint checks whether the attestation issuing authority claim corresponds to one of the accepted values. The wildcard value `*` can be used to accept any issuing authority.

Default: not executed

[source,xml,indent=0]
.Example of IssuingAuthority usage (with WARN validation level)
----
<Attestation>
    ...
    <IssuingAuthority Level="WARN">
        <Id>*</Id>
    </IssuingAuthority>
    ...
</Attestation>
...
* `IssuingAuthorityRegistrationIdentifier` - this constraint checks whether the registration identifier of the attestation issuer corresponds to one of the accepted values. The wildcard value `*` can be used to accept any registration identifier.

Default: not executed

[source,xml,indent=0]
.Example of IssuingAuthorityRegistrationIdentifier usage (with WARN validation level)
----
<Attestation>
    ...
    <IssuingAuthorityRegistrationIdentifier Level="WARN">
        <Id>*</Id>
...
* `OneTimeUse` - this constraint checks whether the attestation token contains a one-time use claim.

Default: `INFORM`

[source,xml,indent=0]
.Example of OneTimeUse usage (with INFORM validation level)
----
<Attestation>
    ...
    <OneTimeUse Level="INFORM" />
    ...
</Attestation>
----

* `ShortLived` - this constraint checks whether the attestation token contains a short-lived claim.

Default: `INFORM`

[source,xml,indent=0]
.Example of ShortLived usage (with INFORM validation level)
----
<Attestation>
    ...
    <ShortLived Level="INFORM" />
    ...
</Attestation>
----

* `RevocationPresent` - this constraint checks whether the attestation token contains a status claim.

Default: `WARN`

[source,xml,indent=0]
.Example of RevocationPresent usage (with WARN validation level)
----
<Attestation>
    ...
    <RevocationPresent Level="WARN" />
    ...
</Attestation>
----

* `RevocationAvailable` - this constraint checks whether an revocation token was obtained for the given attestation, when applicable.

Default: `FAIL`

[source,xml,indent=0]
.Example of RevocationAvailable usage (with FAIL validation level)
----
<Attestation>
    ...
    <RevocationAvailable Level="FAIL" />
    ...
</Attestation>
----

* `AcceptableRevocationFound` - this constraint checks whether a valid and applicable revocation token was found for the given attestation.

Default: `FAIL`

[source,xml,indent=0]
.Example of AcceptableRevocationFound usage (with FAIL validation level)
----
<Attestation>
    ...
    <AcceptableRevocationFound Level="FAIL" />
    ...
</Attestation>
----

* `NotRevoked` - this constraint checks whether the attestation is not revoked.
...
* `NotOnHold` - this constraint checks whether the attestation is not on hold.
...
* `UsePseudonym` - this constraint checks whether the attestation token uses a pseudonym claim.
...
* `Claims` - this constraint checks whether the attestation token contains all claims specified within the list in the constraint. Nested claims are not validated by this constraint.

Default: not executed

[source,xml,indent=0]
.Example of Claims usage (with WARN validation level)
----
<Attestation>
    ...
    <Claims Level="WARN">
...
* `SupportedClaims` - this constraint checks whether the attestation token contains only supported root-level claims. Nested claims are not validated by this constraint.

Default: not executed

[source,xml,indent=0]
.Example of SupportedClaims usage (with WARN validation level)
----
<Attestation>
    ...
    <SupportedClaims Level="WARN">
         <Id>nbf</Id>
         <Id>exp</Id>
         ...
    </SupportedClaims>
...
* `SupportedNamespaces` - this constraint checks whether the attestation token contains only claims within supported namespaces. The constraint is applied only for mdoc validation.

Default: not executed (mdoc only)

[source,xml,indent=0]
.Example of SupportedNamespaces usage (with WARN validation level)
----
<Attestation>
    ...
    <SupportedNamespaces Level="WARN">
...
===== Attestation revocation constraints

The `<AttestationRevocation>` block defines rules for validating a revocation data for applicable attestations. The `<AttestationRevocation>` element shall be a child of `ConstraintsParameters`:

[source,xml,indent=0]
.AttestationRevocation element definition
----
<ConstraintsParameters>
    ...
...
* `Type` - this constraint checks the type of the attestation revocation token (self-claimed).

Default: `FAIL` (accept all)

[source,xml,indent=0]
.Example of Type usage (with FAIL validation level)
----
<Attestation>
    ...
    <Type Level="FAIL">
...
* `UnknownStatus` - this constraint checks the status of the attestation revocation token is not unknown for the given attestation.
...
* `IssuanceTime` - this constraint checks whether the issuance time claim is present within the payload of the attestation revocation token.

Default: `FAIL`

[source,xml,indent=0]
.Example of IssuanceTime usage (with FAIL validation level)
----
<Attestation>
    ...
    <IssuanceTime Level="FAIL" />
    ...
</Attestation>
----

* `ExpirationTime` - this constraint checks whether the expiration time claim is present within the payload of the attestation revocation token.

Default: `FAIL`

[source,xml,indent=0]
.Example of ExpirationTime usage (with FAIL validation level)
----
<Attestation>
    ...
    <ExpirationTime Level="FAIL" />
    ...
</Attestation>
----

* `Subject` - this constraint checks whether the value of the subject claim present in the payload of the attestation revocation token matches the expected value(s).

Default: `FAIL` (accept all)

[source,xml,indent=0]
.Example of Subject usage (with FAIL validation level)
----
<Attestation>
    ...
    <Subject Level="FAIL">
...
* `SubjectMatch` - this constraint checks whether the value of the subject claim present in the payload of the attestation revocation token matches the revocation token's subject extracted from the corresponding attestation's payload claim.

Default: `FAIL`

[source,xml,indent=0]
.Example of SubjectMatch usage (with FAIL validation level)
----
<Attestation>
    ...
    <SubjectMatch Level="FAIL" />
    ...
</Attestation>
----

* `IssuerValidAtIssuanceTime` - this constraint checks whether the issuer of the attestation revocation token was valid at the time of the token issuance.

Default: `FAIL`

[source,xml,indent=0]
.Example of IssuerValidAtIssuanceTime usage (with FAIL validation level)
----
<Attestation>
    ...
    <IssuerValidAtIssuanceTime Level="FAIL" />
    ...
</Attestation>
----
...
.9+|EvidenceRecordConstraints     |EvidenceRecordValid             |LevelConstraint                   |Indication from ER or timestamp |SubIndication from ER or timestamp
                                  ...
                                  |HashTreeRenewal                 |LevelConstraint                   |INDETERMINATE +
                                                                                                       FAILED                |SIGNED_DATA_NOT_FOUND +
                                                                                                                              HASH_FAILURE
                                  |Cryptographic                   |CryptographicConstraint         2+|See CryptographicConstraint
.16+|AttestationConstraints       |AttestationSignatureUnicity     |LevelConstraint                   |FAILED                |FORMAT_FAILURE
                                  |AttestationSignatureValid       |LevelConstraint                   |Indication from signature |SubIndication from signature
                                  ...
                                  |ETSI194721Conformance           |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |AttestationType                 |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |AttestationTypeIntegrityPresent |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |IdentifierPresent               |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |IssuanceDatePresent             |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |otBeforePresent                 |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |ExpirationPresent               |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |NotExpired                      |LevelConstraint                   |INDETERMINATE         |OUT_OF_BOUNDS_NO_POE
.16+|AttestationConstraints       |AdministrativeIssuanceDatePresent        |LevelConstraint          |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |AdministrativeExpirationDatePresent      |LevelConstraint          |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |AdministrativePeriodNotExpired           |LevelConstraint          |INDETERMINATE         |OUT_OF_BOUNDS_NO_POE
                                  |Category                        |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |Subject                         |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |Pseudonym                       |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |IssuingCountry                  |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |IssuingAuthority                |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |IssuingAuthorityRegistrationIdentifier   |LevelConstraint          |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |OneTimeUse                      |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |ShortLived                      |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |RevocationPresent               |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |RevocationAvailable             |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |AcceptableRevocationFound       |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |NotRevoked                      |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |NotOnHold                       |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
.5+|AttestationConstraints        |UsePseudonym                    |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |Claims                          |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |SupportedClaims                 |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |SupportedNamespaces             |LevelConstraint                   |INDETERMINATE         |ATTESTATION_CONSTRAINTS_FAILURE
                                  |Cryptographic                   |CryptographicConstraint         2+|See CryptographicConstraint
