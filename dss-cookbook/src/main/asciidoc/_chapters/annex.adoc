...
DSS allows the user to choose the validation level when performing a signature validation. By default, the highest level (with LTA enabled) is used.
...
===== Signature Policy
The following example illustrates creation of an AdES-BASELINE-B level with a signature policy identifier.
[source,java,indent=0]
...
==== Hash encryption
In order to avoid transferring of original or sensitive information, but also reduce an amount of data by online protocols, a hash of document or a data to be signed can be computed for various purposes.
...
When you want to provide a privacy for original documents, a signature can be created in a detached way, by providing the digest of an original document only. An example of the use case you can find below:
...
When a private key signing is operated remotely, i.e. on an external server, then the document preparation and the actual signing can be made on different servers. An example of such signature creation, when the signature document is created on client's side and the actual signature is made remotely, you can find below:
