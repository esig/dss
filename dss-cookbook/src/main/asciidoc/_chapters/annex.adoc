...
When using provided implementations of the `ASiCContainerMerger`, a user can also benefit from merging a corresponding ASiC container with a simple not-signed ZIP archive, or even from merging two not signed ZIP archives.