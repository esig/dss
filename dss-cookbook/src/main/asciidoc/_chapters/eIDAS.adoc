...
The qualification status determination is based on ETSI TS 119 615 standard (cf. <<R14>>), that requires extraction of information from a certificate (i.e. QcStatements, see <<R24>>) and from a Trusted List (under TSPService corresponding to the certificate).
 ...
 * The certificate type defined in QcStatements (see <<R24>>). One of the following values is supported:
