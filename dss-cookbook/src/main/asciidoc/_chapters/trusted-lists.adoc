...
The CacheCleaner specifies how DSS clears the cache in case of expired URL,... 2 options are available : memory and file-system.
...
An eIDAS XML node linked to the status of the Trusted List is included in the DSS validation policy. The following elements are contained in that node:
