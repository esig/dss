...
The DSS framework allows an independent document timestamping (without a signature). The following Document Signature Services support the timestamping :
...
The code below illustartes a time-stamping process for a PDF document.
...
The code below illustrates a time-stamping process encapsulating the provided documents and the generated time-stamp to an ASiC container.

[source,java,indent=0]
.Timestamping with a container
----
