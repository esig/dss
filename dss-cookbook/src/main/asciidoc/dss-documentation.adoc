...
dss-alerts:: Allows configuration of triggers and handers of custom events.
...
DSS provides an `AbtractAlert` implementation of the interface with a clearly defined structure. The class must be defined with two attributes:
 ...
 * `DSSLogAlert` - allows to define a log level and print of a stacktrace if an exception occurs.

The usage of alerts is available in the following classes:
 * XML-configurators from `dss-jaxb-parsers` module : `TransformerFactoryBuilder`, `SchemaFactoryBuilder`, `ValidatorConfigurator`;
