...
Signature parameters allow enforcing a signature and its signing certificate validation within a creation or extension process.

[[SignXmlXadesBPropertiesTest.java]]
[source,java,indent=0]
.Signature creation requirements enforcement
----
include::{sourcetestdir}/eu/europa/esig/dss/cookbook/example/sign/SignXmlXadesBPropertiesTest.java[tags=requirements]
----

NOTE: The aforementioned requirements, defined within signature parameters, are enforced only on a signature creation or T-level augmentation.

The LT- and LTA- level extension requirements should be defined within <<certificateVerifier>> with <<Alerts>>.
...
The signature service exposes two methods for a counter signature creation. The signature parameters shall take an identifier of a signature to be counter signed.
...
==== XML Trusted List signature

The service provides two methods for a simplified signing of an XML Trusted List with a pre-configured parameters to produce a comformant to the Trusted List specification (cf. <<R11>>) digital signature.

===== Get data to sign

The method allows retrieving the data to be signed. The user sends a full XML Trusted List to be signed, the signing certificate and an optional set of parameters.

WARNING: The parameters in getDataToSign and signDocument MUST be the same (especially the signing date).

.Request
include::_restdocs/tl-signature/1/http-request.adoc[]

.Response
include::_restdocs/tl-signature/1/http-response.adoc[]

===== Sign document

The method allows generation of the signed XML Trusted List with an enveloped signature including the received signature value.

WARNING: The parameters in getDataToSign and signDocument MUST be the same (especially the signing date).

.Request
include::_restdocs/tl-signature/2/http-request.adoc[]

.Response
include::_restdocs/tl-signature/2/http-response.adoc[]
