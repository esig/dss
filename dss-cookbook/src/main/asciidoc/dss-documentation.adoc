...
NOTE: Since version 5.5, DSS supports only physical fonts, that might be embedded to the produced PDF document where necessary. The custom font must be added as an instance of 'DSSDocument' interface to a 'SignatureImageTextParameters' object as following:
...
By default, DSS uses a Google font : 'PT Serif Regular'.
