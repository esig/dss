...
|[[R12]]	R12	|ESI - Procedures for Creation and Validation of AdES Digital Signatures		|ETSI TS 119 102-2		    |1.2.1
|[[R13]]	R13	|ESI - Procedures for using and interpreting EU Member States national trusted lists						|ETSI TS 119 615		    |draft
...
NOTE: We strongly recommend to use the latest available version of JDK, in order to have the latest security fixes and cryptographical algorithm updates.
 ...
 * Java 8 and higher (tested up to Java 14) is required;
...
In order to build a bundle for JDK 14, the following profile can be used from `dss-demo-bundle` module:

----
mvn clean install -P java14
----
...
WARNING: Does not support parallel signatures!
...
As well as a single timestamp creation, DSS provides a validation service for timestamped documents. The timestamp validation process represents "5.4 Time-stamp validation building block" (cf. <<R08>>). The validation process is identical to <<signatureValidation>> process. An appropriate validator will be selected automatically. In total, DSS supports timestamp-alone validation for the following file formats:
...
include::samples/timestamp-detailed-report-example.adoc[]
 ...
 * `Detector` - used to detect an event/state of the object and trigger a process;
 * `AlertHandler` - defines a process to be executed on an object.

In its basic module framework provides a few alerts allowing to handle Exceptions:

 * `ExceptionAlert` - accepts a custom `AlertHandler` to perform actions on an exception if present;
 * `DSSLogAlert` - allows to define a log level and print a stacktrace if an exception occurs.
 ...
 * <<certificateVerifier>> - to handle the raising exception in a custom way (introduced `DSSExceptionAlert` to re-throw exceptions);
...
.Request
include::restdocs/sign-and-extend-one-document/1/http-request.adoc[]

.Response
include::restdocs/sign-and-extend-one-document/1/http-response.adoc[]
...
.Request
include::restdocs/sign-and-extend-one-document/2/http-request.adoc[]

.Response
include::restdocs/sign-and-extend-one-document/2/http-response.adoc[]
...
.Request
include::restdocs/sign-and-extend-one-document/3/http-request.adoc[]

.Response
include::restdocs/sign-and-extend-one-document/3/http-response.adoc[]
...
.Request
include::restdocs/get-keys/1/http-request.adoc[]

.Response
include::restdocs/get-keys/1/http-response.adoc[]
...
.Request
include::restdocs/get-key/1/http-request.adoc[]

.Response
include::restdocs/get-key/1/http-response.adoc[]
...
.Request
include::restdocs/sign-digest-document-remotely/3/http-request.adoc[]

.Response
include::restdocs/sign-digest-document-remotely/3/http-response.adoc[]
...
.Request
include::restdocs/validate-doc/1/http-request.adoc[]

.Response
include::restdocs/validate-doc/1/http-response.adoc[]
...
.Request
include::restdocs/get-original-documents/1/http-request.adoc[]

.Response
include::restdocs/get-original-documents/1/http-response.adoc[]
...
.Request
include::restdocs/validate-cert/1/http-request.adoc[]

.Response
include::restdocs/validate-cert/1/http-response.adoc[]
