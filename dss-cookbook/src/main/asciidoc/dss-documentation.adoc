 ...
 * setProxySource(proxiedSource) : allows definition of an alternative dataLoadet to be used to access a revocation from online sources if a requested revocation is not present in the repository or has been expired (see below).
 * setDefaultNextUpdateDelay(defaultNextUpdateDelay) : if "nextUpdate" field is not defined for a revocation token, the value of "defaultNextUpdateDelay" will be used in order to determine when a new revocation data should be requested. If the current time is not beyond the "thisUpdate" time + "defaultNextUpdateDelay", then a revocation data will be retrieved from the repository source, otherwise a new revocation data will be requested from a proxiedSource. Default : null (a new revocation data will be requested of "nestUpdate" field is not defined).
 * setMaxNextUpdateDelay(maxNextUpdateDelay) : defines a custom maximum possible nextUpdate delay. Allows limiting of a time interval from "thisUpdate" to "nextUpdate" defined in a revocation data. Default : null (not specified, the "nextUpdate" value provided in a revocation is used).
 * setRemoveExpired(removeExpired) : defines if a revocation should be removed on its expiration. Default : true (removes revocation from a repository if expired).
 ...
 * setDataLoader(dataLoader) : allows setting an implementation of `DataLoader` interface, processing a querying of a remote revocation server. `CommonsDataLoader` instance is used by default.
 * setPreferredProtocol(preferredProtocol) : sets a preferred protocol that will be used for obtaining a CRL. E.g. for a list of urls with protocols HTTP, LDAP and FTP, with a defined preferred protocol as FTP, the FTP url will be called first, and in case of an unsuccessful result other url calls will follow. Default : null (urls will be called in a provided order).
 ...
 * setDataLoader(dataLoader) : allows setting an implementation of `DataLoader` interface, processing a querying of a remote revocation server. `CommonsDataLoader` instance is used by default.
 * setNonceSource(nonceSource) : defines an arbitrary integer used in OCSP source querying in order to prevent a replay attack. Default : null (not used by default).
 * setCertIDDigestAlgorithm(certIDDigestAlgorithm) : defines a `DigestAlgorithm` being used to generate a CertificateID in order to complete an OCSP request. OCSP servers supporting multiple hash functions may produce a revocation response with a digest algorithm depending on the provided CertificateID's algorithm. Default : SHA1 (as a mandatory requirement to be implemented by OCSP servers).
...
Several configurations are needed : 

 * the different TLSource(s) and LOTLSource(s) : specify where is the TL/LOTL is hosted and which are the signing certificate(s) for these TL/LOTL. 
 * the TrustedListsCertificateSource : instance of CertificateSource where all trusted certificates and their properties (service type,...) are stored.
 * the DSSFileLoader for the offline refresh (eg : initialization of a webapp at startup from the file cache)
 * the DSSFileLoader for the online refresh (eg : refresh from a cron)
 * the SynchronizationStrategy : specify how to synchronize the trusted list content and the TrustedListsCertificateSource. By default, all trusted lists (expired, invalid,...) will be synchronized.
 * the CacheCleaner (optional) : specify how expired entries are cleaned (memory and/or file-system)
 * the Alert(s) (optional) : allows to generate alert(s) on event(s) (eg : expired trusted list, LOTL location change, expired Official Journal,...)
 * the ExecutorService (optional) : allows to replace the default ExecutorService
 * the debug mode (optional) : prints in the logs the cache status before/after the synchronization.
 
In the next chapter, the different configurations will be covered.
...
`*_dss-detailed-report_*`::
	`*_src/main/java_*`:::
		`*_eu.europa.esig.dss.detailedreport_*`::::
			** `*_DetailedReport.java_*` wrapper(s) which ease the JAXB manipulation
			** `*_..._*`
			** `*_DetailedReportFacade.java_*` class which allows to marshall/unmarshall jaxb objects, generate HTML/PDF content,...
			** `*_DetailedReportXmlDefiner.java_*` class which contains the model definition (XSD, XSLT references, ObjectFactory)
			** `*_jaxb_*` generated on compile time
			 	*** `*_XmlDetailedReport.java_*` JAXB model
				*** `*_..._*`
	`*_src/main/resources_*`:::
		`*_xsd_*`::::
			** `*_DetailedReport.xsd_*` XML Schema (XSD) for the Detailed Report model
			** `*_binding.xml_*` XJC instructions to generate the JAXB model from the XSD
		`*_xslt_*`::::
			** `*_html_*`
			*** 	`*_detailed-report.xslt_*` XML Stylesheet for the HTML generation
			** `*_pdf_*`
			***	`*_detailed-report.xslt_*` XML Stylesheet for the PDF generation
 ...
 * timestampDocument : timestamps a provided document (available for PDF, ASiC-E and ASiC-S container formats)
...
.Request
include::restdocs/timestamp-one-document/http-request.adoc[]

.Response
include::restdocs/timestamp-one-document/http-response.adoc[]
...
This service allows the remote timestamp creation. The method takes as an input the digest to be timestamped and digest algorithm that has been used for the digest value computation. The output of the method is the generated timestamp's binaries.

.Request
include::restdocs/get-timestamp-response/http-request.adoc[]

.Response
include::restdocs/get-timestamp-response/http-response.adoc[]
