...
DSS contains a bundle of JAXB-based modules, generation Java classes in runtime based on XSD-schema. When any change is made in the XSD, the classes of the module are re-generated automatically according to the change. The following modules represent this behavior:
...
To make the documentation available in the Web Application, you will need to process the build instructions. The documentation files will be included to the bundle automatically and available on the address: `http://localhost:8080/doc/dss-documentation.html` and `http://localhost:8080/doc/dss-documentation.pdf` respectively.

===== Javadoc generation

In order to generate HTML Javadoc, you will need to build completely DSS Core as well as DSS Web Application. After this the JavaDoc will be available at the address: `http://localhost:8080/apidocs/index.html`.
