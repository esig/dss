...
public class AttestationValidationContext extends SignatureValidationContext {
     ...
     *
     * @param attestation {@link Attestation} to get status for
     */
    private void findEAAStatusData(Attestation attestation) {
        if (LOG.isTraceEnabled()) {
            LOG.trace("Checking status data for : {}", attestation.getId());
        }
        ...
        if (isEAAStatusCheckRequired(attestation)) {
            ...
            if (LOG.isTraceEnabled()) {
                LOG.trace("EAAPresentation status check is in progress for EAAPresentation : {}", attestation.getId());
            }
